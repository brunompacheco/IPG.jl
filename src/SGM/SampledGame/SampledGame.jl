...
mutable struct GenericSampledGame <: AbstractSampledGame
    S_X::Vector{<:Vector{<:Any}}  # sample of strategies (finite subset of the strategy space X)
end
function GenericSampledGame(players::Vector{<:AbstractPlayer}, S_X::Vector{<:Vector{<:Any}})
...
function add_new_strategy!(sg::GenericSampledGame, players::Vector{<:AbstractPlayer}, new_xp::Vector{<:Any}, p::Integer)
    push!(sg.S_X[p], new_xp)
end

"Compute polymatrix for normal form game from sample of strategies."
function get_polymatrix(players::Vector{<:Player{<:AbstractBilateralPayoff}}, S_X::Vector{<:Vector{<:Any}})
                ...
                for i_p in eachindex(S_X[p])
                    for i_k in eachindex(S_X[k])
                        polymatrix[p,k][i_p,i_k] = (
                            IPG.bilateral_payoff(players[p].Π, S_X[p][i_p], S_X[k][i_k], k)
                            + IPG.bilateral_payoff(players[p].Π, S_X[p][i_p])
                        )
                    end
                end
            end
...
function get_polymatrix(player1::AbstractPlayer, player2::AbstractPlayer, S_X::Vector{<:Vector{<:Any}})
    p = player1.p
    k = player2.p
    ...
    for i_p in eachindex(S_X[p])
        for i_k in eachindex(S_X[k])
            polymatrix[p,k][i_p,i_k] = payoff(player1.Π, S_X[p][i_p], [S_X[k][i_k]])
            polymatrix[k,p][i_k,i_p] = payoff(player2.Π, S_X[k][i_k], [S_X[p][i_p]])
        end
    end
...
function PolymatrixSampledGame(players::Vector{<:Player{<:AbstractBilateralPayoff}}, S_X::Vector{<:Vector{<:Any}})
    return PolymatrixSampledGame(S_X, get_polymatrix(players, S_X))
end
function PolymatrixSampledGame(players::Vector{<:AbstractPlayer}, S_X::Vector{<:Vector{<:Any}})
...
function add_new_strategy!(sg::PolymatrixSampledGame, players::Vector{<:Player{<:AbstractBilateralPayoff}}, new_xp::Vector{<:Any}, p::Integer)
        ...
        elseif (p1 != p) & (p2 == p)
            ...
            for i in 1:strat[p1]
                ...
                sg.polymatrix[p1,p][i,end] = (
                    IPG.bilateral_payoff(players[p1].Π, sg.S_X[p1][i], new_xp, p)
                    + IPG.bilateral_payoff(players[p1].Π, sg.S_X[p1][i])
                )
            end
        elseif (p1 == p) & (p2 != p)
            # add new row to polymatrix to store the utilities wrt the new strategy
            ...
            for i in 1:strat[p2]
                ...
                sg.polymatrix[p,p2][end,i] = (
                    IPG.bilateral_payoff(players[p].Π, new_xp, sg.S_X[p2][i], p2)
                    + IPG.bilateral_payoff(players[p].Π, new_xp)
                )
            end
        end
    end
end
function add_new_strategy!(sg::PolymatrixSampledGame, players::Vector{<:AbstractPlayer}, new_xp::Vector{<:Any}, p::Integer)
    ...
    for i in eachindex(sg.S_X[k])
        ...
        sg.polymatrix[k,p][i,end] = payoff(player_k.Π, sg.S_X[k][i], [new_xp])

        # compute new utilities of player `p` using the new strategy against the old
        # strategies of player `k`
        sg.polymatrix[p,k][end,i] = payoff(player_p.Π, new_xp, [sg.S_X[k][i]])
    end
end
