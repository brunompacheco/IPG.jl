"""
Multiplayer Competitive Facility Location and Design (CFLD) problem.
...
Note that the players have *nonlinear* payoffs. Because there are more than two players, we
follow the utility-based sampled game approach, rather than the polymatrix one (as in
`./cfld.jl`).

# Notes
- SCIP can be replaced with other solvers, such as Gurobi or CPLEX.
- The solver for the sampled games is implemented in `MultiGame.jl`, which uses Gurobi, Juniper,
  and Ipopt, as needed. See `src/SGM/UtilitiesSampledGame/Solve.jl` for details.
"""
...
@constraint(player_A.X, sum(f[i,r] .* x1[i,r] for i in I for r in R) <= B[1])
@constraint(player_A.X, [i in I], sum(x1[i,r] for r in R) <= 1)

player_B = Player(; name="Player B")
@variable(player_B.X, x2[I,R], Bin)
@constraint(player_B.X, sum(f[i,r] .* x2[i,r] for i in I for r in R) <= B[2])
@constraint(player_B.X, [i in I], sum(x2[i,r] for r in R) <= 1)

player_C = Player(; name="Player C")
@variable(player_C.X, x3[I,R], Bin)
@constraint(player_C.X, sum(f[i,r] .* x3[i,r] for i in I for r in R) <= B[3])
@constraint(player_C.X, [i in I], sum(x3[i,r] for r in R) <= 1)

const ε = 1e-3  # small value to prevent division by zero
function cfld_payoff(x_self, x_others)
    self_costs = [sum(u[i,j,r] * x_self[i,r] for i in I for r in R) for j in J]
    others_costs = [sum(u[i,j,r] * x_other[i,r] for i in I for r in R for x_other in x_others) for j in J]
    return sum(
...
set_payoff!(player_A, cfld_payoff(x1, [x2, x3]))
set_payoff!(player_B, cfld_payoff(x2, [x1, x3]))
set_payoff!(player_C, cfld_payoff(x3, [x1, x2]))

# ==== SGM ====
IPG.initialize_strategies = IPG.initialize_strategies_player_alone

Σ, payoff_improvements = SGM([player_A, player_B, player_C], SCIP.Optimizer, max_iter=5, verbose=true)
